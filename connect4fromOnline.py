...
# Find Circles
im, contours, hierarchy = cv2.findContours(edge_detected_image, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE) # Edges to contours
...
if not any([0 in row for row in grid]):
    ...
    move = (adversarialsearch.alpha_beta_cutoff(board, 25, board.heuristic_func), id_yellow)
    print(move)
    ...
    board = Connect4Problem(board=grid)
    move = (adversarialsearch.alpha_beta_cutoff(board, 25, board.heuristic_func), id_red)
    print(move)
